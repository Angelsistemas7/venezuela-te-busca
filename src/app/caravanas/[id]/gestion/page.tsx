...
export default async function CaravanaGestionPage({
  ...
  if (!isOwner) {
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        ...
        <Link href={`/caravanas/${id}`} className="mt-6 inline-block rounded-xl bg-zinc-900 px-5 py-2.5 text-sm font-semibold text-white hover:bg-zinc-800">
          Ver la caravana
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      <Link href={`/caravanas/${id}`} className="mb-5 inline-flex items-center gap-1.5 text-sm font-medium text-zinc-500 hover:text-zinc-900">
